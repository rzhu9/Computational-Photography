...
%H_map = {};
H_map = cell(1, length(H_list)+1);
%H_map_size = size(H_list, 1) + 1;
H11 = eye(3);
H_map{1} = H11;
H_map{2} = H_list{1};
for index = 3:length(H_map)
    H_map{index} = H_map{index-1} * H_list{index-1}; 
end
%------------- YOUR CODE ENDS HERE -----------------
...
imwrite(panorama_image, 'output_images/rzhu9.jpg');
